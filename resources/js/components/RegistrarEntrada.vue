<template>
  <div class="registrar-entrada">
    ...
    <div v-else class="empty-state">
      Selecciona un producto y agrégalo a la tabla para crear la entrada.
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    selectProducto(producto) {
      ...
      this.form.precio_unitario = parseFloat(producto.precio_producto) || 0;
    ...
    async submitEntries() {
      ...
      try {
        ...
        const response = await axios.post('/api/entradas', payload);
        ...
        this.form.precio_unitario = 0;
        this.searchQuery = '';
        this.message = 'Entradas registradas exitosamente';
        this.messageType = 'success';
      } catch (error) {
        console.error('Error al registrar entradas:', error);
        this.message = 'Error al registrar entradas: ' + (error.response?.data?.message || error.message);
        this.messageType = 'error';
      }
    }
  }
